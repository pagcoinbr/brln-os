// Simplified BIP39 implementation for BRLN-OS Wallet
// Based on the BIP39 specification with essential functions only
...
(function(global) {
  ...
  // Simple PBKDF2 implementation using SubtleCrypto API
  ...
  function entropyToMnemonic(entropy) {
    ...
    const hashBuffer = crypto.subtle.digest('SHA-256', entropy);
    
    return hashBuffer.then(hash => {
      const hashArray = new Uint8Array(hash);
      const hashBits = hashArray[0].toString(2).padStart(8, '0').slice(0, checksumBits);
      
      const bits = entropyBits + hashBits;
      const chunks = bits.match(/.{11}/g) || [];
      
      return chunks.map(chunk => WORDLIST[parseInt(chunk, 2)]).join(' ');
    });
  }
  ...
  async function mnemonicToSeed(mnemonic, passphrase = '') {
    ...
    return await pbkdf2(mnemonic, 'mnemonic' + passphrase, 2048, 64);
  }
  ...
  function mnemonicToEntropy(mnemonic) {
    ...
    const entropy = new Uint8Array(entropyBits.match(/.{8}/g).map(byte => 
      parseInt(byte, 2)
    ));
